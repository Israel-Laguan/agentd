...
func TaskSelectColumns(table string) string {
	return fmt.Sprintf(`
		SELECT %s.id, %s.project_id, %s.agent_id, %s.title, %s.description, %s.state, %s.assignee,
		       %s.os_process_id, %s.started_at, %s.completed_at, %s.last_heartbeat, %s.retry_count, %s.token_usage, %s.cached_token_usage, %s.cache_write_token_usage, %s.success_criteria, %s.criteria_met, %s.created_at, %s.updated_at`,
		...
		WHERE state = ?
		  ...
		  AND NOT EXISTS (
		    ...
		    WHERE tr.child_task_id = tasks.id
		      AND tr.relation_type IN (?, ?)
		      AND parent.state != ?
		  )`,
		models.TaskStateReady, FormatTime(now), models.TaskStatePending, parentID,
		models.TaskRelationBlocks, models.TaskRelationDependsOn,
		models.TaskRelationBlocks, models.TaskRelationDependsOn, models.TaskStateCompleted)
	if err != nil {
