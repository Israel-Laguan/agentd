...
func loadAgenticConfig(v *viper.Viper) (AgenticConfig, error) {
	...
	// Batching (inlined from loadBatchingConfig)
	...
	// ToolManifest (inlined from loadToolManifestConfig + loadToolManifestMappings + loadConfidence + interfaceToStringSlice)
		...
		if raw := v.Get("agentic.tool_manifest.mappings"); raw != nil {
			...
			case map[string]interface{}:
				...
				for k, val := range m {
					switch s := val.(type) {
					case []string:
						tmMappings[k] = append([]string(nil), s...)
					case []interface{}:
	...
	tm := ToolManifestConfig{
		Enabled:       tmEnabled,
		MinConfidence: tmMin,
		Mappings:      tmMappings,
	}

	// CapabilityRouting (inlined from loadCapabilityRoutingConfig + loadStringStringMap + loadConfidence)
	...
	crMappings := func() map[string]string {
		key := "agentic.capability_routing.mappings"
		if !v.IsSet(key) {
			return nil
		}
		raw := v.Get(key)
	...
	crTools := func() map[string]string {
		key := "agentic.capability_routing.tools"
		if !v.IsSet(key) {
			return nil
		}
		raw := v.Get(key)
