...
func (s *queueStore) CompleteTieredOrigin(_ context.Context, id string, expectedUpdatedAt time.Time, result models.TaskResult) (*models.Task, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	for i := range s.tasks {
		if s.tasks[i].ID != id {
...
func (s *queueStore) ListParentTasksByRelation(context.Context, string, models.TaskRelationType) ([]models.Task, error) {
	return nil, nil
}
