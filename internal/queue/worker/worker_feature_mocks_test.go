...
func (s *workerTestStore) UpdateTaskPatch(_ context.Context, _ string, _ time.Time, state *models.TaskState, description *string) (*models.Task, error) {
	if state != nil {
		s.task.State = *state
	}
	if description != nil {
...
func (s *workerTestStore) PersistTieredDAG(context.Context, string, time.Time, []models.TieredDAGTask) ([]models.Task, error) {
	return nil, nil
}
...
func (s *workerTestStore) ListParentTasksByRelation(context.Context, string, models.TaskRelationType) ([]models.Task, error) {
	return nil, nil
}
