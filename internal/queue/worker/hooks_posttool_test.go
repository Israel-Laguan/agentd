...
func TestNewWorker_RegistersScrubAndAuditHooks(t *testing.T) {
	...
	tr := w.dispatchToolWithProject(
		context.Background(), "task-int", "proj-int", call, nil, executor, nil, false, nil, nil,
	)
...
func TestAuditHook_DispatchToolEmitsConsistently(t *testing.T) {
	...
	_ = w.dispatchToolWithProject(context.Background(), "task-b", "proj-b", call2, nil, executor, nil, false, nil, nil)
...
func TestErrorPathsRunThroughPostHooks(t *testing.T) {
	...
	tr := w.dispatchToolWithProject(context.Background(), "task-err", "proj-err", call, nil, executor, nil, false, nil, nil)
