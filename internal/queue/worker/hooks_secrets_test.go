...
func TestCredentialInjectionHook_InjectsEnv(t *testing.T) {
	...
	hook := agenthooks.CredentialInjectionHook(store)
...
func TestCredentialInjectionHook_NilStore(t *testing.T) {
	t.Parallel()
	hook := agenthooks.CredentialInjectionHook(nil)
	ctx := agenthooks.HookContext{
		ToolName:  "github",
...
func TestCredentialValidationSessionHook_PassesWhenSet(t *testing.T) {
	...
	store := wsession.NewEnvSecretStore(map[string]string{"github": envKey})
	hook := agenthooks.CredentialValidationSessionHook(store)
	err := hook.Fn(agenthooks.HookContext{})
	if err != nil {
		t.Fatalf("hook returned error: %v", err)
	}
}
...
func TestCredentialValidationSessionHook_NilStore(t *testing.T) {
	t.Parallel()
	hook := agenthooks.CredentialValidationSessionHook(nil)
	err := hook.Fn(agenthooks.HookContext{})
	if err != nil {
...
func TestCredentials_NeverAppearInAuditPayload(t *testing.T) {
	...
	hc.RegisterPre(agenthooks.CredentialInjectionHook(store))
	hc.RegisterPre(agenthooks.CredentialDetectionHook())
...
func TestCredentials_DoNotLeakAcrossToolCalls(t *testing.T) {
	...
	githubVerdict := hc.RunPre(agenthooks.HookContext{
		ToolName:  "github",
		Args:      `{"repo":"org/repo"}`,
		CallID:    "call-iso-1",
	...
	gitlabVerdict := hc.RunPre(agenthooks.HookContext{
		ToolName:  "gitlab",
	...
	executor := agenttools.NewToolExecutor(nil, t.TempDir(), []string{"BASE=1"}, 0)
...
func TestHookChain_SessionStart_CredentialValidation(t *testing.T) {
	...
	store := wsession.NewEnvSecretStore(map[string]string{"github": envKey})
	hc := agenthooks.NewHookChain()
	hc.RegisterSessionStart(agenthooks.CredentialValidationSessionHook(store))

	err := hc.RunSessionStart(agenthooks.HookContext{})
	if err != nil {
