...
func TestGroupTurns_AssistantAfterToolSameTurn(t *testing.T) {
	...
	if len(turns[0].Messages) != 3 {
		t.Fatalf("expected 3 messages in turn 0, got %d", len(turns[0].Messages))
	}
	if len(turns[1].Messages) != 1 {
		t.Fatalf("expected 1 message in turn 1, got %d", len(turns[1].Messages))
	}
}

func TestGroupTurns_AgenticToolCycles(t *testing.T) {
	cm := &ContextManager{}
	messages := []spec.PromptMessage{
		{Role: "user", Content: "user1"},
		{Role: "assistant", Content: "ast1", ToolCalls: []spec.ToolCall{{ID: "1"}}},
		{Role: "tool", ToolCallID: "1", Content: "res1"},
		{Role: "assistant", Content: "ast2", ToolCalls: []spec.ToolCall{{ID: "2"}}},
	...
	for i, want := range wantLens {
		if len(turns[i].Messages) != want {
			t.Errorf("turn %d: expected %d messages, got %d", i, want, len(turns[i].Messages))
		}
	}
}
