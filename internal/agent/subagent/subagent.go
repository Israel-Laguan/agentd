...
func (d *SubagentDelegate) Delegate(
	...
	toolExec := agenttools.NewToolExecutor(d.sandbox, d.workspacePath, d.envVars, d.wallTimeout)
...
func (d *SubagentDelegate) runSubagentStep(
	...
	def SubagentDefinition, toolExec *agenttools.ToolExecutor,
) bool {
	...
	*messages = append(*messages, gateway.PromptMessage{
	...
	for _, call := range resp.ToolCalls {
		...
		if call.Function.Name == agenttools.ToolNameWrite {
...
func enforceSubagentContextBudget(messages []gateway.PromptMessage, budget int) []gateway.PromptMessage {
	if budget <= 0 || agentcontext.TotalChars(messages) <= budget {
		return messages
	}
	if len(messages) <= 2 {
		return messages
	...
	remaining := budget - agentcontext.TotalChars(fixed)
	if remaining <= 0 {
		return fixed
	}
	return append(fixed, agentcontext.TruncateWorkingMessages(messages[2:], remaining)...)
}
