...
| `ProviderUsed` identity | `ProviderUsed` in API responses and the `provider supports chat tools` startup log reflects the provider's `name` from the `gateway.providers` entry (e.g. `"groq"`, `"poolside"`). For the built-in `openai` slot, `ProviderUsed` is always `"openai"` regardless of `gateway.openai.base_url` — so an OpenAI adapter pointed at a third-party endpoint still reports `"openai"`. Custom named entries use their `name` field. See [`docs/openai-compatible-providers.md`](openai-compatible-providers.md). |
