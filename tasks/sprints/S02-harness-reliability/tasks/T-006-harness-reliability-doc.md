...
- [ ] `docs/harness-reliability.md` Beat 1 (restart mid-task) has pass criteria — no silent stuck `RUNNING`, same `--home` resume
- [ ] Commands or fixtures listed: `kill <agentd-pid>` (unclean) → `agentd start --skip-llm-warmup` with same `--home` → `GET /api/v1/projects/{id}/tasks` + `GET /api/v1/system/status` before/after
