...
- S01 retro actions: README First Run now leads with LiteLLM/Poolside/mock instead of stale Gemini-only (`README.md:41`); SP-004 pre-flight done; **unresolved carry: default reverted to `gemini-2.5-flash` after discovering `gemini-2.0-flash` was shut down, but SP-003 documents that default as returning 404 — T-012 required a valid id or empty model + order-only cascade, and the PR's stated goal was gemini-2.0-flash, so the default is still the known-stale value.**
