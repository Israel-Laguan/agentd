...
| [SP-006](spikes/SP-006-atomic-tiered-origin-completion.md) | spike | Atomic tiered-origin completion | ready | P1 |
| [BUG-002](bugs/BUG-002-context-step-committed-description.md) | bug | Context step reads unpopulated committed.Description | ready | P1 |
| [BUG-001](bugs/BUG-001-missing-web-interface.md) | bug | Missing web interface / dashboard | ready | P3 |
...
1. **PR 1: Bug fixes & State Cleanup (~15–25 files)**
   - **Covers:** `SP-006` (Atomic completion), `BUG-002` (Context step bug), `BUG-001` (Web/health endpoints), `T-021` (Drop `task.Logs`).
...
2. **PR 2: Observability + MCP Export (~25–40 files)**
   ...
   - **Why:** The debug logging adds `slog.Debug` calls at key subsystem boundaries (intake, router, gateway, worker loop) — silent by default, enabled via `log_level: debug` or `-v`. No correlation ID plumbing or signature changes needed. The MCP export introduces a new package (`internal/mcp/`). Shipping both together lets us use the new debug logs to validate the MCP integration end-to-end in a single review cycle.
