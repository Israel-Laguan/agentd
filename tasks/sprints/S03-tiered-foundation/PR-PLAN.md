...
| **PR-B** | `feat/s03-tiered-m2-contextpack` | T-008 | ≤15 | <800 | ContextPack schema + store/load + read-only context step + `*_test.go`; size-limit enforcement. No splitter, no allowlists, no escalation |
| **PR-C** | `docs/s03-disk-watchdog-beat` | T-013 | ≤10 | <600 | `docs/harness-reliability.md` (Beat 2.3), optional `scripts/demo/disk-watchdog.sh`, `internal/queue/disk_watchdog_test.go` + features only. **No** prod change to `disk_watchdog.go` — gap found → split to follow-up ticket |
| **PR-D** | `docs/s03-memory-recall-beat` | T-014 | ≤10 | <600 | `docs/harness-reliability.md` (Beat 2.4), `internal/memory/**` recall/librarian `*_test.go` + features only, optional `scripts/demo/memory-recall.sh`. **No** prod change — gap found → split to follow-up ticket |
