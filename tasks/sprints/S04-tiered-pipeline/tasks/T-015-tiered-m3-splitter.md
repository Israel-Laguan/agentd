...
- [x] Gate passes: `ShouldRunTiered(task)` returns true → splitter runs — gate already shipped in M1/M2 (`internal/queue/worker/phase_splitter.go`); `SplitIntoTieredDAG` is the pure constructor callers invoke after the gate passes
...
- [x] Tests: gate pass produces children, gate fail stays one-shot, dependency order correct — gate behavior already covered by `phase_splitter_test.go`'s `TestShouldRunTiered_*` suite (untouched); splitter output covered by `splitter_test.go`
...
- `SplitIntoTieredDAG` is a **pure, in-memory constructor** — no `*Worker` receiver, no store writes. Real persistence of `SPAWNED_BY`/`DEPENDS_ON` children (the only current relation writer, `InsertTaskRelation`, hardcodes `BLOCKS` and lives in `internal/kanban`, out of PR-A scope) and dispatch wiring are T-016's job.
