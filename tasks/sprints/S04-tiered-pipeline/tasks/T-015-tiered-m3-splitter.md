...
- [ ] Gate passes: `ShouldRunTiered(task)` returns true → splitter runs — gate shipped in M1/M2 (`internal/queue/worker/phase_splitter.go`); `SplitIntoTieredDAG` constructor written and tested; dispatch wiring lands in T-016
...
- T-016 will integrate the splitter into the worker dispatch path after the `ShouldRunTiered` gate and own step-kind dispatch and mode tool allowlists
