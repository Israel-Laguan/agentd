...
- [ ] Context mode: reads task + workspace, writes ContextPack to workspace, attaches pack path to child tasks
...
- ContextPack is the sealed handoff — decision/execute/verify inject it via system prompt
- Tool allowlists can reuse the existing `ToolManifest` / `AllowedTools` mechanism
