...
func writeBaseline(path string, violations []violation) error {
	...
	if err == nil {
		...
		return writeBaselineFile(path, violations, true)
	}
	if !os.IsNotExist(err) {
		return err
	}
...
func writeBaselineDir(path string, violations []violation) error {
	...
	for start := 0; start < len(violations); start += baselineChunkSize {
		...
		if err := writeBaselineFile(filepath.Join(path, name), violations[start:end], false); err != nil {
			return err
		}
	}
	return nil
...
func writeBaselineFile(path string, violations []violation, withHeader bool) error {
	...
	if withHeader {
		out.WriteString("# minfunc baseline - accepted functions with < 3 lines.\n")
